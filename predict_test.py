...
class VideoDescriptionInference(object):
    ...
    def greedy_search(self, loaded_array):
        ...
        encoder_outputs, h, c = self.inf_encoder_model.predict(loaded_array.reshape(-1, 80, 2048))
        target_seq = np.zeros((1, 1, 1500))
        sentence = ''
        target_seq[0, 0, self.tokenizer.word_index['bos']] = 1
        for i in range(15):
            output_tokens, h, c = self.inf_decoder_model.predict([target_seq, encoder_outputs, h, c],verbose=0)
            states_value = [h, c]
    ...
    def decode_sequence2bs(self, input_seq):
        encoder_outputs, h, c = self.inf_encoder_model.predict(input_seq)
        states_value = [encoder_outputs, h, c]
    ...
    def beam_search(self, target_seq, states_value, prob, path, lens):
        ...
        global decode_seq
        node = 2
        encoder_outputs, prev_h, prev_c = states_value

        output_tokens, h, c = self.inf_decoder_model.predict([target_seq, encoder_outputs, prev_h, prev_c],verbose=0)
        output_tokens = output_tokens.reshape(self.num_decoder_tokens)
        ...
        with open(os.path.join(self.test_path, 'test_%s.txt' % self.search_type), 'w') as file:
            for idx, x in enumerate(X_test):
                ...
                if self.search_type == 'greedy':
                    start = time.time()
                    decoded_sentence = self.greedy_search(x.reshape(-1, 80, 2048))
                    file.write(decoded_sentence + ',{:.2f}'.format(time.time()-start))
                else:
                    start = time.time()
                    decoded_sentence = self.decode_sequence2bs(x.reshape(-1, 80, 2048))
